...
## Build SGSR in UE
This release contains 3 methods of SGSR:
- Spatial Upscaler
- Temporal Upscaler
...
Command line:
	r.SGSR.Method 0
```
Enable SGSR Temporal Upscaling:
    ...
    TU Setting:
        r.SGSR.Quality(refer to SGSRTU.cpp): Temporal Upscale quality mode(TU only). Default is 1(Quality: 1.5x).
            Available:
                ...
                2 - Balanced 			1.7x
                3 - Performance 		2.0x
            For example: device resolution is 2400x1080, r.SGSR.Quality=1, will upscale from 1600x720 to 2400x1080.
...
r.SGSR.Enabled=1
```
Select desired upscaling method(switching between SU and TU will automatically set corresponding AA methods, no need to set it manually):
```text
r.SGSR.Method=1
```
...
AMD-derived temporal shaders also carry MIT terms. See the [third-party notices](THIRD-PARTY-NOTICES.txt).
